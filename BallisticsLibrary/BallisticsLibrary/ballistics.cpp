...
int main()
{
   ProjectilePath* aProjectilePath = new ProjectilePath[BCOMP_MAXRANGE];
   ...
   double RelativeHumidity = 0.78; // The relative humidity fraction.  Ranges from 0.00 to 1.00, with 0.50 being 50% relative humidity.
	...
	double sightToBoreAngle = CalculateSightToBoreAngle(G1,ballisticCoefficient,initialVelocity,sightHeightOverBore,zeroRange,yIntercept);
	...
	int rowCount = SolveAll(G1,ballisticCoefficient,initialVelocity,sightHeightOverBore,boreAngle,sightToBoreAngle,windspeed,windangle,aProjectilePath);

	// Now print a simple chart of X / Y trajectory spaced at 10yd increments
	for (int s = 0; s <= 100; s++)
	{
		printf("\nX: %.0f     Y: %.2f		t: %.2f", aProjectilePath[s*10].Range, aProjectilePath[s*10].Path, aProjectilePath[s*10].Time);
	}
...
double CalculateFactorR(double Temperature, double Pressure, double RelativeHumidity)
{
...
double DragCoefficientAtmosphericCorrection(double DragCoefficient, double Altitude_feet, double Barometer_hg, double Temperature_f, double RelativeHumidity)
{
...
double CalculateSightToBoreAngle(int DragFunction, double DragCoefficient, double Velocity_feet_sec, double SightHeight_inch, double ZeroRange_yard, double yIntercept_inch)
{
   ...
   {
		vy = Velocity_feet_sec * sin(angle);
		vx = Velocity_feet_sec * cos(angle);
...
int SolveAll(int DragFunction, double DragCoefficient, double Velocity_feet_sec, double SightHeight_inch, double BoreAngle, double SightToBoreAngle, double WindSpeed_mile_hr, double WindAngle, ProjectilePath aProjectilePath[])
{
      // seconds
		double t = 0;
		double dt = 0;
      ...
      // feet per second
      double Gy = GRAVITY * cos(DegtoRad((BoreAngle + SightToBoreAngle)));
		double Gx = GRAVITY * sin(DegtoRad((BoreAngle + SightToBoreAngle)));

		vx = Velocity_feet_sec * cos(DegtoRad(SightToBoreAngle));
		vy = Velocity_feet_sec * sin(DegtoRad(SightToBoreAngle));
		...
		for (t = 0; ; t = t + dt)
		{
         ...
         v = sqrt(vx*vx+vy*vy);
			...
			if (x/3>=n)
			{
				aProjectilePath[n].Range = x/3;							                                 // Range in yards
				aProjectilePath[n].Path = y*12;							                                 // Path in inches
				aProjectilePath[n].MOA = -RadtoMOA(atan(y/x));			                              // Correction in MOA
				aProjectilePath[n].Time = t+dt;							                                 // Time in s
				aProjectilePath[n].Windage = WindageCorrection(crosswind_mile_hr,Velocity_feet_sec,x,t+dt);  // Windage in inches
            aProjectilePath[n].WindageMOA = RadtoMOA(atan(aProjectilePath[n].Windage/(12*x)));  // Windage in MOA
				aProjectilePath[n].Velocity = v;								                              // Velocity (combined)
				aProjectilePath[n].Vx = vx;								                                 // Velocity (x)
				aProjectilePath[n].Vy = vy;								                                 // Velocity (y)
				n++;	
			}
...
double DragRetardationVelocity(int DragFunction, double DragCoefficient, double Velocity_feet_sec)
{
